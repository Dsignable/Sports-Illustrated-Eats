...
/**
 * Fix news page redirection and template loading
 */
function si_fix_news_page_redirect() {
    // Add rewrite rule for news page
    add_rewrite_rule(
        '^news/?$',
...
add_action('init', 'si_fix_news_page_redirect');

/**
 * Fix news page template loading
 */
function si_fix_news_page_template($template) {
    if (is_page('news')) {
        $new_template = locate_template(array('page-news.php'));
...
add_filter('template_include', 'si_fix_news_page_template');

/**
 * Flush rewrite rules when needed
 */
function si_flush_rewrite_rules() {
    // Only flush once per session
    if (!get_option('si_rewrite_rules_flushed')) {
        flush_rewrite_rules();
        update_option('si_rewrite_rules_flushed', true);
    }
}
add_action('init', 'si_flush_rewrite_rules', 20);
...
function si_header_navigation_customizer($wp_customize) {
    ...
    // Menu Dropdown URLs - Old hidden heading (removing)
    ...
    // Add a heading for the menu dropdown section
    if (class_exists('WP_Customize_Control')) {
        class SI_Customize_Heading_Control extends WP_Customize_Control {
            public $type = 'heading';
            
            public function render_content() {
                if (!empty($this->label)) {
                    echo '<h3 style="margin-top:30px;border-bottom:1px solid #ccc;padding-bottom:10px;color:#0073aa;">' . esc_html($this->label) . '</h3>';
                }
    ...
    // Add a proper heading for the menu dropdown section
    $wp_customize->add_setting('si_menu_dropdown_section_heading', array(
        'default' => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    
    $wp_customize->add_control(new SI_Customize_Heading_Control($wp_customize, 'si_menu_dropdown_section_heading', array(
        'label'       => __('Menu Dropdown URLs', 'sports-illustrated'),
        'description' => __('Configure the URLs for each menu dropdown item.', 'sports-illustrated'),
        'section'     => 'si_header_navigation',
        'priority'    => 90,
    ...
    foreach ($menu_items as $key => $label) {
        ...
        $wp_customize->add_setting('si_menu_dropdown_separator_' . $key, array(
            'default' => '',
            'sanitize_callback' => 'sanitize_text_field',
        ));
        
        $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_menu_dropdown_separator_' . $key, array(
            'section'     => 'si_header_navigation',
            'type'        => 'hidden',
            'input_attrs' => array(

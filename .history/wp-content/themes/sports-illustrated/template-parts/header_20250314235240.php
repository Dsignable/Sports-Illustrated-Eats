                ...
                if (get_theme_mod('si_left_menu_visible_' . $i, true)) {
                    ...
                    if ($i === 1) {
                        ...
                        $menu_types = array(
                            'full' => array(
                                'label' => 'Full Menu',
                                'url' => home_url('/?page_id=50&menu=full'),
                                'fallback' => home_url('/?page_id=50')
                            ),
                            'drink' => array(
                                'label' => 'Drink Menu',
                                'url' => home_url('/?page_id=50&menu=drink'),
                                'fallback' => home_url('/?page_id=50&menu=drink')
                            ),
                            'brunch' => array(
                                'label' => 'Brunch Menu',
                                'url' => home_url('/?page_id=50&menu=brunch'),
                                'fallback' => home_url('/?page_id=50&menu=brunch')
                            ),
                            'happy' => array(
                                'label' => 'Happy Hour',
                                'url' => home_url('/?page_id=50&menu=happy'),
                                'fallback' => home_url('/?page_id=50&menu=happy')
                            ),
                            'today' => array(
                                'label' => 'Today\'s Menu',
                                'url' => home_url('/?page_id=50&menu=today'),
                                'fallback' => home_url('/?page_id=50&menu=today')
                            )
                        );
                        foreach ($menu_types as $key => $menu_item) {
                            $menu_url = get_theme_mod('si_menu_dropdown_url_' . $key, $menu_item['fallback']);
                            echo '<a href="' . esc_url($menu_url) . '" class="dropdown-item" data-menu-type="' . esc_attr($key) . '">' . esc_html($menu_item['label']) . '</a>';
                        }
                        echo '</div></div>';
                    } else {

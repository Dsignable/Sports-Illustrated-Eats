...
<div class="navigation-bar">
    ...
    <nav class="desktop-menu left-menu">
        ...
        // Left Menu Items
        for ($i = 1; $i <= 3; $i++) {
            if (get_theme_mod('si_nav_left_menu_visible_' . $i, true)) {
                $menu_text = get_theme_mod('si_nav_left_menu_text_' . $i, $i == 1 ? 'MENU' : ($i == 2 ? 'RESERVATIONS' : ($i == 3 ? 'ORDER ONLINE' : '')));
                $menu_url = get_theme_mod('si_nav_left_menu_url_' . $i, $i == 1 ? home_url('/menu/') : ($i == 2 ? home_url('/reservations/') : ($i == 3 ? 'https://www.order.store/store/sports-illustrated-clubhouse/k003zVjYXe6DFY58dmcpCw' : '')));
                $has_dropdown = get_theme_mod('si_nav_left_menu_has_dropdown_' . $i, $i == 1);
                ...
                if ($has_dropdown) {
                    ...
                    foreach ($menu_types as $key => $default_label) {
                        ...
                        if (get_theme_mod('si_nav_menu_dropdown_visible_' . $key, true)) {
                            // Get customized text from theme mod, fallback to default
                            $label = get_theme_mod('si_nav_menu_dropdown_text_' . $key, $default_label);
                            
                            // Get the custom URL directly from theme mod
                            $menu_url = get_theme_mod('si_nav_menu_dropdown_url_' . $key, '');
    ...
    <nav class="desktop-menu right-menu">
        ...
        // Right Menu Items
        for ($i = 1; $i <= 3; $i++) {
            $menu_text = get_theme_mod('si_nav_right_menu_text_' . $i, $i == 1 ? 'CATERING & EVENTS' : ($i == 2 ? 'NEWS' : ($i == 3 ? 'CONTACT US' : '')));
            ...
            if ($i == 2) { // News button
                ...
                $menu_url = get_theme_mod('si_nav_right_menu_url_' . $i, $i == 1 ? home_url('/catering/') : ($i == 2 ? home_url('/club-news/') : ($i == 3 ? home_url('/contact/') : '')));
            }
            
            $menu_visible = get_theme_mod('si_nav_right_menu_visible_' . $i, $i <= 3);
            $has_dropdown = get_theme_mod('si_nav_right_menu_has_dropdown_' . $i, $i == 3);
            ...
            if ($menu_visible && $menu_text) {
                if ($has_dropdown) {
                    ...
                    // Dropdown items
                    for ($j = 1; $j <= 5; $j++) {
                        $dropdown_text = get_theme_mod('si_nav_right_menu_dropdown_' . $i . '_text_' . $j, '');
                        $dropdown_url = get_theme_mod('si_nav_right_menu_dropdown_' . $i . '_url_' . $j, '');
                        $dropdown_visible = get_theme_mod('si_nav_right_menu_dropdown_' . $i . '_visible_' . $j, false);

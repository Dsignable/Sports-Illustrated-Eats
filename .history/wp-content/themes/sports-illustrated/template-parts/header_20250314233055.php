            ...
            for ($i = 1; $i <= 3; $i++) {
                if (get_theme_mod('si_left_menu_visible_' . $i, true)) {
                    ...
                    if ($i === 1) {
                        ...
                        // Menu dropdown items with hardcoded fallbacks
                        $menu_types = array(
                            'full' => array(
                                'label' => 'Full Menu',
                                'url' => get_theme_mod('si_menu_dropdown_url_full', home_url('/menu/?type=full')),
                                'fallback' => home_url('/menu/?type=full')
                            ),
                            'drink' => array(
                                'label' => 'Drink Menu',
                                'url' => get_theme_mod('si_menu_dropdown_url_drink', home_url('/menu/?type=drink')),
                                'fallback' => home_url('/menu/?type=drink')
                            ),
                            'brunch' => array(
                                'label' => 'Brunch Menu',
                                'url' => get_theme_mod('si_menu_dropdown_url_brunch', home_url('/menu/?type=brunch')),
                                'fallback' => home_url('/menu/?type=brunch')
                            ),
                            'happy' => array(
                                'label' => 'Happy Hour',
                                'url' => get_theme_mod('si_menu_dropdown_url_happy', home_url('/menu/?type=happy')),
                                'fallback' => home_url('/menu/?type=happy')
                            ),
                            'today' => array(
                                'label' => 'Today\'s Menu',
                                'url' => get_theme_mod('si_menu_dropdown_url_today', home_url('/menu/?type=today')),
                                'fallback' => home_url('/menu/?type=today')
                            )
                        );

                        foreach ($menu_types as $key => $menu) {
                            $menu_url = !empty($menu['url']) ? $menu['url'] : $menu['fallback'];
                            echo '<a href="' . esc_url($menu_url) . '" class="dropdown-item" data-menu-type="' . esc_attr($key) . '">' . esc_html($menu['label']) . '</a>';
                        }
                        echo '</div></div>';
                    } else {

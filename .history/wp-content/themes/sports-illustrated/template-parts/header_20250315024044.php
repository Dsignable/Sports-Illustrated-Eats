                ...
                $menu_url = get_theme_mod('si_left_menu_url_' . $i, $i == 1 ? home_url('/menu/') : ($i == 2 ? home_url('/reservations/') : ($i == 3 ? 'https://www.order.store/store/sports-illustrated-clubhouse/k003zVjYXe6DFY58dmcpCw' : '')));
                    ...
                    if ($i === 1) {
                        ...
                        foreach ($menu_types as $key => $default_label) {
                            ...
                            if ($menu_page_id) {
                                // Try to get custom URL from theme mod
                                $custom_url = get_theme_mod('si_menu_dropdown_url_' . $key, '');
                                
                                if (!empty($custom_url)) {
                                    $menu_url = $custom_url;
                                } else {
                                    // Fallback to default URL structure with page_id
            ...
            for ($i = 1; $i <= 3; $i++) {
                ...
                if ($i == 2) { // News button
                    ...
                    if ($posts_page_id) {
                        ...
                        // Try to find a page with slug 'club-news'

...
function si_header_navigation_customizer($wp_customize) {
    ...
    foreach ($menu_items as $key => $label) {
        ...
        // Menu dropdown item URL - Make sure this is properly displayed
        ...
        // Explicitly add the URL control with clear labeling
        $wp_customize->add_control('si_menu_dropdown_url_' . $key, array(
            ...
            'priority'    => 20,
            'input_attrs' => array(
                'placeholder' => home_url('/menu/?menu=' . $key),
                'class'       => 'widefat',
            ),
        ));
        
        // Add visibility control for each menu dropdown item
        $wp_customize->add_setting('si_menu_dropdown_visible_' . $key, array(
            'default'           => true,
            'sanitize_callback' => 'rest_sanitize_boolean',
            'transport'         => 'postMessage',
        ));
        
        $wp_customize->add_control('si_menu_dropdown_visible_' . $key, array(
            'label'       => sprintf(__('Show %s in dropdown', 'sports-illustrated'), $label),
            'section'     => 'si_header_navigation',
            'type'        => 'checkbox',
            'priority'    => 20,
        ));

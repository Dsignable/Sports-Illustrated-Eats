...
function si_header_navigation_customizer($wp_customize) {
    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        if ($i === 1) {
            // Menu dropdown items heading
            $wp_customize->add_setting('si_left_menu_dropdown_heading', array(
                'default'           => '',
                'sanitize_callback' => 'sanitize_text_field',
            ));
            
            $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_left_menu_dropdown_heading', array(
                'label'       => __('Menu Dropdown Items', 'sports-illustrated'),
                'description' => __('Configure the text and URLs for each menu dropdown item.', 'sports-illustrated'),
                'section'     => 'si_header_navigation',
                'type'        => 'hidden',
            ...
            foreach ($menu_items as $key => $label) {
                ...
                // URL control - immediately after the text control

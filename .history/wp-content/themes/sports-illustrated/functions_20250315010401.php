...
function si_header_navigation_customizer($wp_customize) {
    ...
    // Menu Dropdown URLs - Section Heading
    ...
    $priority = 21; // Start priority after the heading
    
    foreach ($menu_items as $key => $label) {
        // Create a group heading for each menu item
        $wp_customize->add_setting('si_menu_dropdown_group_' . $key, array(
            'default'           => '',
            'sanitize_callback' => 'sanitize_text_field',
        ));
        
        $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_menu_dropdown_group_' . $key, array(
            'label'       => sprintf(__('%s Settings', 'sports-illustrated'), $label),
            'section'     => 'si_header_navigation',
            'type'        => 'hidden',
            'priority'    => $priority,
        )));
        ...
        $priority += 2; // Add extra space between menu items
    }
}

...
function si_header_navigation_customizer($wp_customize) {
    ...
    foreach ($menu_items as $key => $label) {
        // Section heading for each menu item
        $wp_customize->add_setting('si_menu_dropdown_heading_' . $key, array(
            'default'           => '',
            'sanitize_callback' => 'sanitize_text_field',
        ));
        
        $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_menu_dropdown_heading_' . $key, array(

...
function si_scripts() {
    ...
    wp_enqueue_script(
        ...
        filemtime(get_theme_file_path('/assets/js/menu-pages.js')),
        true
    );
...
function si_header_navigation_customizer($wp_customize) {
    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        // Has Dropdown
        $wp_customize->add_setting('si_left_menu_has_dropdown_' . $i, array(
            'default'           => $i == 1, // Default true for Menu (1st item)
            'sanitize_callback' => 'rest_sanitize_boolean',
        ));

        $wp_customize->add_control('si_left_menu_has_dropdown_' . $i, array(
            'label'    => sprintf(__('Enable Dropdown for Left Menu Item %d', 'sports-illustrated'), $i),
            'section'  => 'si_header_navigation',
            'type'     => 'checkbox',
        ));
        
        // If this is the first item (Menu), add dropdown menu items
        if ($i == 1) {
            // Menu dropdown enable
            $wp_customize->add_setting('si_menu_dropdown_enable', array(
                'default'           => true,
                'sanitize_callback' => 'rest_sanitize_boolean',
            ));
            
            $wp_customize->add_control('si_menu_dropdown_enable', array(
                'label'    => __('Enable Menu Dropdown', 'sports-illustrated'),
                'section'  => 'si_header_navigation',
                'type'     => 'checkbox',
                'active_callback' => function() {
                    return get_theme_mod('si_left_menu_has_dropdown_1', true);
                },
            ));
            ...
            $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_menu_dropdown_heading', array(
                ...
                'type'        => 'hidden',
                'active_callback' => function() {
                    return get_theme_mod('si_left_menu_has_dropdown_1', true) && get_theme_mod('si_menu_dropdown_enable', true);
                },
            )));
            ...
            foreach ($menu_items as $key => $label) {
                ...
                $wp_customize->add_control('si_menu_dropdown_text_' . $key, array(
                    ...
                    'section'     => 'si_header_navigation',
                    'type'        => 'text',
                    'priority'    => 10 + (array_search($key, array_keys($menu_items)) * 10),
                    'active_callback' => function() {
                        return get_theme_mod('si_left_menu_has_dropdown_1', true) && get_theme_mod('si_menu_dropdown_enable', true);
                    },
                ));
                ...
                $wp_customize->add_control('si_menu_dropdown_url_' . $key, array(
                    ...
                    'section'     => 'si_header_navigation',
                    'type'        => 'url',
                    'priority'    => 11 + (array_search($key, array_keys($menu_items)) * 10),
                    'active_callback' => function() {
                        return get_theme_mod('si_left_menu_has_dropdown_1', true) && get_theme_mod('si_menu_dropdown_enable', true);
                    },
                ));
    ...
    // The separate menu dropdown settings have been integrated with the left menu items
}
*/
// Disabled in favor of the new Navigation Bar Settings
...
function si_footer_customizer_settings($wp_customize) {
    ...
    $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_footer_social_heading', array(
        ...
        'section'     => 'si_footer_section',
        'settings'    => 'si_footer_social_heading',
        'type'        => 'hidden',
    )));
    ...
    $social_platforms = array(
        'instagram' => array(
            ...
            'default_url' => 'https://www.instagram.com/si_clubhouse/'
        ),
        'twitter' => array(
            'label' => 'X (Twitter)',
    ...
    for ($i = 1; $i <= 5; $i++) {
        // Platform Selection
        $wp_customize->add_setting('si_footer_social_' . $i . '_platform', array(
            'default'           => $i === 1 ? 'instagram' : '',
            'sanitize_callback' => 'sanitize_text_field',
        ));

        $platform_choices = array('' => __('Select Platform', 'sports-illustrated'));
        foreach ($social_platforms as $key => $platform) {
            $platform_choices[$key] = $platform['label'];
        }

        $wp_customize->add_control('si_footer_social_' . $i . '_platform', array(
        ...
        // URL
        $wp_customize->add_setting('si_footer_social_' . $i . '_url', array(
            'default'           => $i === 1 ? $social_platforms['instagram']['default_url'] : '',
            'sanitize_callback' => 'esc_url_raw',
        ));

        $wp_customize->add_control('si_footer_social_' . $i . '_url', array(
        ...
        // Visibility
        $wp_customize->add_setting('si_footer_social_' . $i . '_visible', array(
            'default'           => $i === 1,
            'sanitize_callback' => 'rest_sanitize_boolean',
        ));

        $wp_customize->add_control('si_footer_social_' . $i . '_visible', array(
...
function si_get_social_icon($platform) {
    $social_platforms = array(
    ...
    return isset($social_platforms[$platform]) ? $social_platforms[$platform] : 'fab fa-globe';
}
...
function si_get_image_url($theme_mod_key, $default = '') {
    ...
    // If no default was provided and we're looking for a news placeholder,
    // use a placeholder service
    if (empty($default) && strpos($theme_mod_key, 'news') !== false) {
        return 'https://via.placeholder.com/800x450?text=News+Placeholder';
    }
...
function si_menu_gallery_customizer($wp_customize) {
    // Only define the control class if it doesn't exist and WP_Customize_Control exists
    if (!class_exists('SI_Gallery_Control') && class_exists('WP_Customize_Control')) {
        /**
         * Custom Gallery Control Class for the Customizer
         */
        class SI_Gallery_Control extends WP_Customize_Control {
            public $type = 'gallery';
            ...
            public function render_content() {
                ...
                <div class="gallery-images-container">
                        ...
                        if (!empty($image_ids)) {
                            foreach ($image_ids as $image_id) {
                                ...
                                if ($image) {
                                    ...
                                    echo '<a href="#" class="remove-image"><span class="dashicons dashicons-no-alt"></span></a>';
                                    echo '</div>';
                                }
                            }
                        }
                    ...
                    <button type="button" class="button add-gallery-images"><?php _e('Add Images', 'sports-illustrated'); ?></button>
                </div>
                ...
                <script>
                    jQuery(document).ready(function($) {
                        ...
                        $('.add-gallery-images').on('click', function(e) {
                            ...
                            var $container = $(this).closest('.gallery-images-container');
                            var $input = $container.find('input[type="hidden"]');
                            var $selectedImages = $container.find('.selected-images');
                            ...
                            frame = wp.media({
                                title: '<?php _e('Select Gallery Images', 'sports-illustrated'); ?>',
                                button: {
                                    text: '<?php _e('Add to Gallery', 'sports-illustrated'); ?>'
                                },
                                multiple: true
                            });
                                ...
                                selection.map(function(attachment) {
                                    ...
                                    if (attachment.id) {
                                        ...
                                        $selectedImages.append(
                                            ...
                                            '<a href="#" class="remove-image"><span class="dashicons dashicons-no-alt"></span></a>' +
                                            '</div>'
                                        );
                                    }
                                });
                        ...
                        $(document).on('click', '.remove-image', function(e) {
                            ...
                            var $wrapper = $(this).closest('.gallery-image-wrapper');
                            var $container = $wrapper.closest('.gallery-images-container');
                            var $input = $container.find('input[type="hidden"]');
                        ...
                        $('.selected-images').sortable({
                            ...
                            update: function() {
                                ...
                                $(this).find('.gallery-image-wrapper').each(function() {
...
/**
 * Enqueue admin scripts and styles for the customizer.
 */
function si_enqueue_customizer_scripts() {
    // Enqueue jQuery UI Sortable for the gallery control
    wp_enqueue_script('jquery-ui-sortable');
}
add_action('customize_controls_enqueue_scripts', 'si_enqueue_customizer_scripts');
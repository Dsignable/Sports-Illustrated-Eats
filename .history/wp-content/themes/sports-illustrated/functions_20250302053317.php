...
function si_menu_page_settings($wp_customize) {
    // Menu Page Section
    ...
    // Background Image
    $wp_customize->add_setting('si_menu_page_bg', array(
        'default'           => '',
        'sanitize_callback' => 'absint',
    ));

    $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, 'si_menu_page_bg', array(
        'label'     => __('Menu Page Background Image', 'sports-illustrated'),
        'section'   => 'si_menu_page_section',
        'settings'  => 'si_menu_page_bg',
        'mime_type' => 'image',
    )));
    ...
    foreach ($menu_types as $key => $label) {
        ...
        $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, 'si_menu_' . $key . '_image', array(
            ...
            'section'   => 'si_menu_page_section',
            'settings'  => 'si_menu_' . $key . '_image',
            'mime_type' => 'image',
        )));
    }
}
add_action('customize_register', 'si_menu_page_settings');

// Helper function to get menu image URL
function si_get_menu_image_url($menu_type) {
    $attachment_id = get_theme_mod('si_menu_' . $menu_type . '_image');
    if ($attachment_id) {
...
// Helper function to get menu page background
function si_get_menu_background() {
    $attachment_id = get_theme_mod('si_menu_page_bg');
    if ($attachment_id) {
        return wp_get_attachment_image_url($attachment_id, 'full');
    }
    return '';

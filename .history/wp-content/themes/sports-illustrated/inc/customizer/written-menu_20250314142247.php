...
function si_written_menu_customizer_settings($wp_customize) {
    ...
    foreach ($menu_types as $type => $label) {
        ...
        $wp_customize->add_control(new WP_Customize_Redirect_Control($wp_customize, "si_written_menu_{$type}_redirect", array(
            ...
            'description' => __('Restaurant menus are managed directly in the customizer.', 'sports-illustrated'),
            'button_text' => __('Edit Menu Settings', 'sports-illustrated'),
            'button_url'  => admin_url('customize.php?autofocus[panel]=si_menus_panel'),
            'priority'    => 10,
        )));
    }
}

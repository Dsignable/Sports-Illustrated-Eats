...
class SI_Menu_Manager {
    ...
    private function reset_menu($menu_type) {
        ...
        if (function_exists('si_set_default_menu_values')) {
            ...
            // Now get all theme mods
            $theme_mods = get_theme_mods();
            
            // Keep only the theme mods for this menu type and remove others
            foreach ($theme_mods as $key => $value) {
                // If this is a menu setting but not for our target menu type, remove it
                if (strpos($key, 'si_written_menu_') === 0 && 
                    strpos($key, "si_written_menu_{$menu_type}_") !== 0 &&
                    strpos($key, 'si_written_menu_full_') !== 0 &&
                    strpos($key, 'si_written_menu_drink_') !== 0 &&
                    strpos($key, 'si_written_menu_brunch_') !== 0) {
                    remove_theme_mod($key);
                }
            }
            
            // Now we need to restore the other menu types
            foreach ($this->menu_types as $other_type => $label) {
                if ($other_type !== $menu_type) {
                    $this->restore_other_menu($other_type);
                }
            }
        }
    }
    ...
    /**
     * Restore a menu that wasn't being reset
     *
     * @param string $menu_type Menu type to restore
     */
    private function restore_other_menu($menu_type) {
        // Get all theme mods
        $theme_mods = get_theme_mods();
        
        // Backup the current menu settings
        $backup = array();
        foreach ($theme_mods as $key => $value) {
            if (strpos($key, "si_written_menu_{$menu_type}_") === 0) {
                $backup[$key] = $value;

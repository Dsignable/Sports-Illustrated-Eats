...
function si_header_navigation_customizer($wp_customize) {
    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        // Add a divider for dropdown items if dropdown is enabled
        if ($i == 3) { // Only for the default dropdown (Contact Us)
            $wp_customize->add_setting('si_right_menu_dropdown_divider_' . $i, array(
                'default'           => '',
                'sanitize_callback' => 'sanitize_text_field',
            ));
            
            $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_right_menu_dropdown_divider_' . $i, array(
                'label'       => sprintf(__('▼ Dropdown Items for %s', 'sports-illustrated'), get_theme_mod('si_right_menu_text_' . $i, $i == 3 ? 'CONTACT US' : '')),
                'description' => __('Configure the dropdown items that appear under this menu item', 'sports-illustrated'),
                'section'     => 'si_header_navigation',
                'settings'    => 'si_right_menu_dropdown_divider_' . $i,
                'type'        => 'hidden',
        ...
        for ($j = 1; $j <= 5; $j++) {
            ...
            $wp_customize->add_control('si_right_menu_dropdown_' . $i . '_url_' . $j, array(
                ...
                'section'  => 'si_header_navigation',
                'type'     => 'text', // Changed from 'url' to 'text' for better compatibility
                'input_attrs' => array(
                    'placeholder' => 'https://',
                    'class'       => 'widefat',
                ),
    ...
    $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_menu_dropdown_heading', array(
        ...
        'description' => __('Configure the text and URLs for each menu dropdown item under the MENU button.', 'sports-illustrated'),
        'section'     => 'si_header_navigation',
        'type'        => 'hidden',
        'priority'    => 20,
    )));
    
    // Add a notice about URL fields
    $wp_customize->add_setting('si_menu_dropdown_url_notice', array(
        'default'           => '',
        'sanitize_callback' => 'sanitize_text_field',
    ));
    
    $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_menu_dropdown_url_notice', array(
        'label'       => __('⚠️ Important: URL Configuration', 'sports-illustrated'),
        'description' => __('Each menu item below has both a Display Text field and a URL field. Make sure to set both for each item.', 'sports-illustrated'),
        'section'     => 'si_header_navigation',
        'settings'    => 'si_menu_dropdown_url_notice',
        'type'        => 'hidden',
        'priority'    => 20,
    )));
    ...
    foreach ($menu_items as $key => $label) {
        // Add a section divider for each menu item
        $wp_customize->add_setting('si_menu_dropdown_divider_' . $key, array(
            'default'           => '',
            'sanitize_callback' => 'sanitize_text_field',
        ));
        
        $wp_customize->add_control(new WP_Customize_Control($wp_customize, 'si_menu_dropdown_divider_' . $key, array(
            'label'       => sprintf(__('▼ %s Options', 'sports-illustrated'), $label),
            'description' => sprintf(__('Configure the "%s" dropdown item', 'sports-illustrated'), $label),
            'section'     => 'si_header_navigation',
            'settings'    => 'si_menu_dropdown_divider_' . $key,
            'type'        => 'hidden',
            'priority'    => 20,
        )));
        ...
        $wp_customize->add_control('si_menu_dropdown_text_' . $key, array(
            'label'       => sprintf(__('%s Display Text', 'sports-illustrated'), $label),
            'description' => __('The text shown in the dropdown menu', 'sports-illustrated'),
            'section'     => 'si_header_navigation',
            'type'        => 'text',
            'priority'    => 20,
        ));
        
        // Add a separate URL field that will definitely display
        $url_setting_id = 'si_menu_dropdown_url_' . $key;
        $wp_customize->add_setting($url_setting_id, array(
            'default'           => home_url('/menu/?menu=' . $key),
            'sanitize_callback' => 'esc_url_raw',
        ));
        
        $wp_customize->add_control($url_setting_id, array(
            'label'       => sprintf(__('%s URL', 'sports-illustrated'), $label),
            'description' => __('Enter the full URL where this menu item should link to', 'sports-illustrated'),
            'section'     => 'si_header_navigation',
            'type'        => 'text',
            'priority'    => 20,
        ));

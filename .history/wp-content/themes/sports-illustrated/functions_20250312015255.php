...
function si_display_thumbnail_column($column_name, $post_id) {
    if ('thumbnail' === $column_name) {
        if (has_post_thumbnail($post_id)) {
            echo '<a href="' . esc_url(get_edit_post_link($post_id)) . '#postimagediv">';
            echo get_the_post_thumbnail($post_id, array(80, 80));
            echo '</a>';
        } else {
            echo '<a href="' . esc_url(get_edit_post_link($post_id)) . '#postimagediv" class="set-post-thumbnail">';
            echo '<span style="display:inline-block; width:80px; height:80px; background:#f0f0f0; border:1px dashed #ccc; text-align:center; line-height:80px; color:#999;">';
            echo __('Set Image', 'sports-illustrated');
            echo '</span>';
            echo '</a>';
        }
    }
...
function si_featured_image_management_page_callback() {
                    ...
                    <tbody>
                        <?php foreach ($posts_without_featured_image as $post) : ?>
                            <tr>
                                ...
                                <td>
                                    <a href="<?php echo get_edit_post_link($post->ID); ?>#postimagediv" class="button-secondary"><?php _e('Set Featured Image', 'sports-illustrated'); ?></a>
                                    <button type="button" class="button-secondary set-single-featured-image" data-post-id="<?php echo esc_attr($post->ID); ?>"><?php _e('Quick Set', 'sports-illustrated'); ?></button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
    ...
    jQuery(document).ready(function($) {
        ...
        var defaultFeaturedImageFrame;
        
        $('#select-default-featured-image').click(function(e) {
            e.preventDefault();
            
            if (defaultFeaturedImageFrame) {
                defaultFeaturedImageFrame.open();
                return;
            }
            
            defaultFeaturedImageFrame = wp.media({
            ...
            defaultFeaturedImageFrame.on('select', function() {
                var attachment = defaultFeaturedImageFrame.state().get('selection').first().toJSON();
    ...
    jQuery(document).ready(function($) {
        ...
        $('.column-thumbnail').each(function() {
            ...
            if (cell.find('img').length === 0) {
                ...
                // Add placeholder
                cell.html('<div class="no-thumbnail"></div>');

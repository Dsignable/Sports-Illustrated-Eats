...
function si_menu_customizer_settings($wp_customize) {
    ...
    $wp_customize->add_section('si_menu_cards', array(
        'title'    => __('Menu Card Images', 'sports-illustrated'),
        'priority' => 31,
    ));

    // Happy Hour Card Image
    $wp_customize->add_setting('si_menu_card_happy_hour', array(
        'default'           => '',
        'sanitize_callback' => 'absint',
        'transport'         => 'refresh'
    ));

    $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, 'si_menu_card_happy_hour', array(
        'label'         => __('Happy Hour Card Image', 'sports-illustrated'),
        'section'       => 'si_menu_cards',
        'mime_type'     => 'image',
    )));

    // Fan Favorites Card Image
    $wp_customize->add_setting('si_menu_card_fan_favorites', array(
        'default'           => '',
        'sanitize_callback' => 'absint',
        'transport'         => 'refresh'
    ));

    $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, 'si_menu_card_fan_favorites', array(
        'label'         => __('Fan Favorites Card Image', 'sports-illustrated'),
        'section'       => 'si_menu_cards',
        'mime_type'     => 'image',
    )));

    // Drinks & Cocktails Card Image
    $wp_customize->add_setting('si_menu_card_drinks_cocktails', array(
        'default'           => '',
        'sanitize_callback' => 'absint',
        'transport'         => 'refresh'
    ));

    $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, 'si_menu_card_drinks_cocktails', array(
        'label'         => __('Drinks & Cocktails Card Image', 'sports-illustrated'),
        'section'       => 'si_menu_cards',
        'mime_type'     => 'image',
    )));
    ...
    // Menu Images
    $menu_images = array(
        'full' => 'Full Menu Image',
        'happy' => 'Happy Hour Menu Image',
        'drink' => 'Drink Menu Image',
        'monday' => 'Monday Menu Image',
        'tuesday' => 'Tuesday Menu Image',
        'wednesday' => 'Wednesday Menu Image',
        'thursday' => 'Thursday Menu Image',
        'friday' => 'Friday Menu Image',
        'saturday' => 'Saturday Menu Image',
        'sunday' => 'Sunday Menu Image'
    );

    foreach ($menu_images as $key => $label) {
        $wp_customize->add_setting('si_menu_' . $key . '_image', array(
            'default'           => '',
            'sanitize_callback' => 'absint',
            'transport'         => 'refresh'
        ));

        $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, 'si_menu_' . $key . '_image', array(
...
/**
 * Get menu card image URL
 */
function si_get_menu_card_image($card_type) {
    $attachment_id = get_theme_mod('si_menu_card_' . $card_type);
    if ($attachment_id) {

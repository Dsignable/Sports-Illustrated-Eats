...
function sports_illustrated_setup() {
    ...
    // Set default featured image size for news posts
...
function si_add_thumbnail_column($columns) {
    $columns['thumbnail'] = __('Featured Image', 'sports-illustrated');
    return $columns;
}
...
function si_display_thumbnail_column($column_name, $post_id) {
    if ('thumbnail' === $column_name) {
        if (has_post_thumbnail($post_id)) {
            echo '<img src="' . esc_url(get_the_post_thumbnail_url($post_id, 'thumbnail')) . '" style="max-width:100px; height:auto;" />';
        } else {
            echo '<span style="color:#999;">' . __('No featured image set', 'sports-illustrated') . '</span>';
        }
    }
}
...
function si_menu_page_customizer($wp_customize) {
    ...
    $wp_customize->add_setting('si_menu_size_multiplier', array(
        'default'           => '1',
        'sanitize_callback' => 'sanitize_text_field',
    ));

    $wp_customize->add_control('si_menu_size_multiplier', array(
        'label'       => __('Menu Size Multiplier', 'sports-illustrated'),
        'description' => __('Increase the size of all menu images to make them more legible. Higher values make menus larger.', 'sports-illustrated'),
    ...
    foreach ($menu_types as $menu_id => $menu_name) {
        ...
        $wp_customize->add_control(new WP_Customize_Media_Control($wp_customize, 'si_restaurant_menu_' . $menu_id . '_pdf', array(
            'label'    => $menu_name . ' PDF',
            'description' => sprintf(__('Upload a PDF for the %s.', 'sports-illustrated'), $menu_name),
            'section'  => 'si_menu_page_section',

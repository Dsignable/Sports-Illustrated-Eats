...
function si_written_menu_customizer_settings($wp_customize) {
    ...
    $wp_customize->add_panel('si_menus_panel', array(
        'title'    => __('Menus', 'sports-illustrated'),
        'priority' => 130,
    ));
    ...
    foreach ($menu_types as $type => $label) {
        ...
        // Add reset button for drinks menu
        if ($type === 'drinks') {
            $wp_customize->add_setting('si_reset_drinks_menu_button', array(
                'default'           => '',
                'sanitize_callback' => 'sanitize_text_field',
            ));
            
            $wp_customize->add_control(new WP_Customize_Button_Control($wp_customize, 'si_reset_drinks_menu_button', array(
                'label'       => __('Reset Drinks Menu to Defaults', 'sports-illustrated'),
                'description' => __('Click to reset the drinks menu to the default values with proper subsections.', 'sports-illustrated'),
                'section'     => "si_written_menu_{$type}_section",
                'settings'    => 'si_reset_drinks_menu_button',
                'priority'    => 5,
                'button_text' => __('Reset Drinks Menu', 'sports-illustrated'),
                'button_url'  => admin_url('themes.php?page=reset-drinks-menu'),
            )));
        }
        ...
        for ($i = 1; $i <= 10; $i++) {
            ...
            // Add items for this section (up to 15 items per section)
            for ($j = 1; $j <= 15; $j++) {
                // Item name

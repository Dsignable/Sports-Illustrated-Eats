...
class SI_Menu_Manager {
    ...
    private function set_menu_defaults($menu_type) {
        ...
        // Call the function to set defaults for this menu type
        if (function_exists('si_set_default_menu_values')) {
            // Use the existing function to reset all menus
            delete_option('si_default_menus_set');
            si_set_default_menu_values();
            ...
            // Keep only the theme mods for this menu type
            foreach ($theme_mods as $key => $value) {
                // If this is a menu setting but not for our target menu type, remove it
                if (strpos($key, 'si_written_menu_') === 0 && strpos($key, "si_written_menu_{$menu_type}_") !== 0) {
                    remove_theme_mod($key);
                }
            }
        }

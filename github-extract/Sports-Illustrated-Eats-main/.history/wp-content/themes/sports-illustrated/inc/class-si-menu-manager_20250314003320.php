...
class SI_Menu_Manager {
    ...
    private function reset_menu($menu_type) {
        ...
        if (function_exists('si_set_default_menu_values')) {
            ...
            // Now remove the other menu types we want to preserve
            $other_menus = array_keys($this->menu_types);
            foreach ($other_menus as $other_menu) {
                if ($other_menu !== $menu_type) {
                    foreach ($theme_mods as $key => $value) {
                        if (strpos($key, "si_written_menu_{$other_menu}_") === 0) {
                            remove_theme_mod($key);
                        }
                    }
                }
    ...
    private function set_menu_defaults($menu_type) {
        // This method is no longer needed as we're using reset_menu
        return;
    }
}

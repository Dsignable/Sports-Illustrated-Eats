...
function si_page_backgrounds_customizer($wp_customize) {
    ...
    $wp_customize->add_section('si_page_backgrounds', array(
        ...
        'description' => __('Configure background images for various pages throughout the site.', 'sports-illustrated'),
        'priority' => 30,
    ));

    // Background opacity
    $wp_customize->add_setting('si_page_bg_opacity', array(
        'default'           => '0.5',
        'sanitize_callback' => 'sanitize_text_field',
    ));

    $wp_customize->add_control('si_page_bg_opacity', array(
        'label'    => __('Background Overlay Opacity', 'sports-illustrated'),
        'description' => __('Set the opacity of the dark overlay on background images (0.0 = fully transparent, 1.0 = fully opaque).', 'sports-illustrated'),
        'section'  => 'si_page_backgrounds',
        'type'     => 'text',
    ));

    // Background color
    $wp_customize->add_setting('si_page_bg_color', array(
        'default'           => '#000000',
        'sanitize_callback' => 'sanitize_hex_color',
    ));

    $wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, 'si_page_bg_color', array(
        'label'    => __('Background Overlay Color', 'sports-illustrated'),
        'description' => __('Choose the color of the overlay on background images.', 'sports-illustrated'),
        'section'  => 'si_page_backgrounds',
    )));
}

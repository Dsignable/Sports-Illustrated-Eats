        ...
        <nav class="desktop-menu left-menu">
            ...
            for ($i = 1; $i <= 3; $i++) {
                if (get_theme_mod('si_left_menu_visible_' . $i, true)) {
                    ...
                    if ($i === 1) {
                        // Get the menu page ID dynamically
                        $menu_page = get_page_by_path('menu');
                        $menu_page_id = $menu_page ? $menu_page->ID : 50; // Fallback to 50 if not found

                        echo '<div class="menu-dropdown-wrapper">';
                        echo '<a href="' . esc_url(add_query_arg('page_id', $menu_page_id, home_url('/'))) . '" class="nav-button has-dropdown">' . esc_html($menu_text) . '</a>';
                        echo '<div class="menu-dropdown">';
                        // Menu dropdown items with dynamic page ID
                        $menu_types = array(
                            'full' => 'Full Menu',
                            'drink' => 'Drink Menu',
                            'brunch' => 'Brunch Menu',
            ...
            for ($i = 1; $i <= 3; $i++) {
                if (get_theme_mod('si_left_menu_visible_' . $i, true)) {
                    ...
                    if ($i === 1) {
                        ...
                        echo '<div class="menu-dropdown-wrapper">';
                        echo '<a href="' . esc_url(add_query_arg('page_id', $menu_page_id, home_url('/'))) . '" class="has-dropdown">' . esc_html($menu_text) . '</a>';
                        echo '<div class="menu-dropdown">';
                        // Menu dropdown items with dynamic page ID
                        $menu_types = array(
                            'full' => 'Full Menu',

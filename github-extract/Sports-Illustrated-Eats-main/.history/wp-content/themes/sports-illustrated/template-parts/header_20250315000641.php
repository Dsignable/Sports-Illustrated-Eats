    ...
    <div class="navigation-bar">
                ...
                if (get_theme_mod('si_left_menu_visible_' . $i, true)) {
                    ...
                    if ($i === 1) {
                        ...
                        foreach ($menu_types as $key => $label) {
                            $menu_url = get_page_by_path('menu') ? 
                                add_query_arg('menu', $key, get_permalink(get_page_by_path('menu'))) : 
                                add_query_arg('menu', $key, home_url('/menu/'));
        ...
        <nav class="desktop-menu right-menu">
            ...
            for ($i = 1; $i <= 3; $i++) {
                ...
                if ($i == 2) { // News button
                    $menu_url = get_permalink(get_option('page_for_posts')); // Get the posts page URL
                    if (!$menu_url) {
                        $menu_url = home_url('/news/'); // Fallback URL
                    }
                } else {

...
function si_get_menu_card_image($card_id, $default = '') {
    error_log('=== Menu Card Image Debug Start ===');
    error_log('Card ID: ' . $card_id);
    ...
    if ($attachment_id) {
        $image_url = wp_get_attachment_image_url($attachment_id, 'full');
        error_log('Image URL from attachment: ' . var_export($image_url, true));
        
        if ($image_url) {
            error_log('Returning attachment image URL');
            return $image_url;
        }
    }
    
    error_log('Default Image: ' . var_export($default, true));
    error_log('=== Menu Card Image Debug End ===');
...
function si_get_menu_card_link($card_id) {
    error_log('=== Menu Card Link Debug Start ===');
    error_log('Card ID: ' . $card_id);
    
    $link = get_theme_mod('si_menu_card_' . $card_id . '_link', '#');
    error_log('Link from theme mod: ' . var_export($link, true));
    
    if (empty($link)) {
        error_log('Link is empty, returning default #');
        $link = '#';
    }
    
    error_log('=== Menu Card Link Debug End ===');
    return $link;
}

// Add default images for menu cards if not set
function si_set_default_menu_card_images() {
    $menu_cards = array(
        'happy_hour' => 'happy-hour.jpg',
        'fan_favorites' => 'fan-favorites.jpg',
        'drinks_cocktails' => 'drinks-cocktails.jpg'
    );
    
    foreach ($menu_cards as $card_id => $image_name) {
        if (!get_theme_mod('si_menu_card_' . $card_id . '_image')) {
            $default_image = get_theme_file_uri('assets/images/menu-cards/' . $image_name);
            set_theme_mod('si_menu_card_' . $card_id . '_image', $default_image);
        }
    }
}
add_action('after_setup_theme', 'si_set_default_menu_card_images');

// Add debugging for theme mods
function si_debug_theme_mods() {
    error_log('=== All Theme Mods Debug ===');
    $theme_mods = get_theme_mods();
    error_log(print_r($theme_mods, true));
    error_log('=== End Theme Mods Debug ===');
}
add_action('init', 'si_debug_theme_mods');
...
// Remove any duplicate functions or actions
remove_action('customize_register', 'si_landing_page_menu_cards');
remove_action('customize_register', 'si_menu_cards_customizer_duplicate'); 
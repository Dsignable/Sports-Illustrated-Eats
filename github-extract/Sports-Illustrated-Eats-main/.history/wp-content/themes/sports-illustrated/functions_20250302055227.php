...
function si_get_menu_card_image($card_id, $default = '') {
    ...
    if ($attachment_id) {
        $image_url = wp_get_attachment_image_url($attachment_id, 'full');
        error_log('Found Image URL: ' . var_export($image_url, true));
        if ($image_url) {
            return $image_url;
        }
    }
    
    // If no image is set, use default images based on card type
    $default_images = array(
        'happy_hour' => get_theme_file_uri('assets/images/menu-cards/happy-hour.jpg'),
        'fan_favorites' => get_theme_file_uri('assets/images/menu-cards/fan-favorites.jpg'),
        'drinks_cocktails' => get_theme_file_uri('assets/images/menu-cards/drinks-cocktails.jpg')
    );
    
    if (isset($default_images[$card_id])) {
        error_log('Using default image for ' . $card_id);
        return $default_images[$card_id];
    }
    
    error_log('No image found, using provided default');
    return $default;
}

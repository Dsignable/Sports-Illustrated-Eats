...
/**
 * Add rewrite rules for menu pages
 */
function si_add_menu_rewrite_rules() {
    // Add rewrite rules for menu types
    add_rewrite_rule(
        'menu/([^/]+)/?$',
        'index.php?pagename=menu&menu=$matches[1]',
        'top'
    );
    
    // Register the 'menu' query var
    global $wp;
    $wp->add_query_var('menu');
}
add_action('init', 'si_add_menu_rewrite_rules');
...
/**
 * Fix menu URLs to prevent 404 errors
 */
function si_fix_menu_urls($redirect_url, $requested_url) {
    // Don't redirect URLs with menu parameter
